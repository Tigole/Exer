...
void Quest_Main::mt_Get_Description(std::vector<std::vector<sf::String>>& description)
{
    ...
    if (l_Dark_Mage != nullptr)
    {
        ...
        {
            description.push_back({"J'ai résolu le mystère de l'attaque des villageois."});
        }
        else
        {
            description.push_back({"Les villageois de " + fn_City_Name("La Passe") + " ont été attaqués.",
    ...
    if (l_Troll != nullptr)
    {
        if (l_Troll->m_Completed == true)
        {
            ...
            l_Text.push_back("J'ai réussi à traverser le pont.");

            if (l_Troll->m_Paid == true)
            {
                l_Text.push_back("J'ai perdu ma pièce d'or mais j'en trouverai certainement d'autres.");
            }
            else
            {
                l_Text;
        ...
        {
            description.push_back({"Un troll m'empêche de traverser le pont..."});
        }
    }
    ...
    if (l_Tutoriel != nullptr)
    {
        ...
        {
            description.push_back({"J'ai terminé le tutoriel !"});
        }
        else
        {
...
void Quest_Troll::mt_Get_Description(std::vector<std::vector<sf::String>>& description)
{
    if (m_Completed == true)
    {
        description.push_back({"J'ai réussi à passer le pont."});
    }

    description.push_back({"Un troll me barre la route."});
    description.push_back({"Je peux me battre contre lui.","Il me semble également que les trolls appécient l'or."});
}
...
void Quest_Troll::mt_On_Victory(void)
{
    ...
    l_Script->mt_Add_Command(new Command_ShowDialog({fn_Dialog(l_Player, "Merci à vous."),
                                                    fn_Dialog(l_Player, "Mais vous savez, je n'ai fait que mon devoir."),
                                                    fn_Dialog(l_Germain, "On peut enfin aller à " + fn_City_Name("Axâme") + ".")}));
...
bool Quest_Dark_Mage::mt_On_Interaction(std::vector<Resource<Dynamic>>& dyns, Dynamic* tgt, Quest_Nature nature)
{
    ...
    else if ((m_Started == true) && (m_Villager_Help == false) && (m_Villager_Seen == false) && ((tgt->m_Name == l_Villageois2->m_Name) || (tgt->m_Name == l_Bob->m_Name)))
    {
        ...
        else
        {
            l_Script->mt_Add_Command(new Command_ShowDialog({fn_Dialog(l_Villageois2, "Désolé pour lui."),
                                                            fn_Dialog(l_Player, "Ne vous en faites pas.\nJ'ai traversé moultes épreuves plus torturantes."),
                                                            fn_Dialog(l_Player, "J'ai réglé votre problème au sanctuaire."),
                                                            fn_Dialog(l_Villageois2, "C'et vrai ?"),
                                                            fn_Dialog(l_Villageois2, "Merci énormément !"),
                                                            fn_Dialog(l_Villageois2, "Mais qu'est ce qu'il y avait ?"),
                                                            fn_Dialog(l_Player, "Un homme.\nUn mage."),
                                                            fn_Dialog(l_Player, "Il n'est plus en état de vous nuire.\nC'est tout ce qui compte."),
                                                            fn_Dialog(l_Villageois2, "Et bien merci à vous.")}));
            ...
            if (m_Mage_Defeated_Before_Herbe = false)
            {
                ...
                if (Context::smt_Get().m_Engine->mt_Get_Creature("Nivalys")->m_Gameplay_Data.m_Health <= 0)
                {
                    l_Script->mt_Add_Command(new Command_ShowDialog({fn_Dialog(l_Player, "A vrai dire.\nIl n'y a plus rien à voir\nau sanctuaire."),
                                                                    fn_Dialog(l_Player, "Je me suis occupé du mage qui vous a attaqué."),
                                                                    fn_Dialog(l_Villageois2, "Grand Direg !"),
                                                                    fn_Dialog(l_Villageois2, "C'est une excellente nouvelle !"),
                                                                    fn_Dialog(l_Villageois2, "Merci à vous étranger !"),
    ...
    else if ((tgt->m_Name == "SombreManigances_Mage") && (m_Mage_Seen == false))
    {
        ...
        if ((m_Troll_Help == true) && (m_Villager_Help == true))
        {
            ...
            l_Script->mt_Add_Command(new Command_ShowDialog({fn_Dialog(l_Villageois2, "Je ne pensais pas être là avec toi.\nMais c'est une bonne chose que tu sois là."),
...
void Quest_Dark_Mage::mt_On_Victory(void)
{
    ...
    {
        l_Script->mt_Add_Command(new Command_ShowDialog({fn_Dialog(l_Villageois, "Ça c'était pour Armand !")}));
    }
    if ((m_Troll_Help == true) && (l_Troll->m_Gameplay_Data.m_Health > 0))
    {
        l_Script->mt_Add_Command(new Command_Creature_LookAt(l_Troll, l_Player));
        l_Script->mt_Add_Command(new Command_ShowDialog({fn_Dialog(l_Troll, "Je m'attendais à plus menaçant...")}));
    }
...
void Quest_Dark_Mage::mt_Get_Description(std::vector<std::vector<sf::String>>& description)
{
    if (m_Completed == true)
    {
        description.push_back({"J'ai élucidé le mystère du sanctuaire et sauver les villageois !"});
    }

    if (m_Troll_Asked == true)
    {
        description.push_back({"Le troll m'attend au sanctuaire\npour régler cette affaire au clair"});
    }

    if (m_Villager_Help == true)
    {
        description.push_back({"J'ai trouvé la plante.",
    ...
    else
    {
        ...
        {
            description.push_back({"Le villageois pas blessé m'a proposé son aide.",
        ...
        {
            description.push_back({"Le troll pourrait peut-être m'aider également ?"});
        }
    }

    if (m_Started == true)
    {
        description.push_back({"Des villageois se sont fait attaqué\nalors qu'ils étaient au sanctuaire."});
    }
...
void Quest_Tutoriel::mt_Get_Description(std::vector<std::vector<sf::String>>& description)
{
    if (m_Completed == true)
    {
        description.push_back({"J'ai fini cette quête !"});
    }

    if (m_Chest_Open == true)
    {
        description.push_back({"J'ai trouvé un coffre au bout du chemin\net une pièce d'or à l'intérieur.",
                              "Je me demande à quoi elle pourra me servir."});
    }
    ...
    {
        description.push_back({"Le loup n'est plus un problème."});
    }
    else if (m_Wolf_Seen == true)
    {
        description.push_back({"Un loup se trouve en travers de mon chemin.",
                              "Je peux essayer de passer à côté.",
                              "Sait-on jamais, ça peut marcher ?"});
    }
}
